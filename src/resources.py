...
class Resource:
    ...
    def type_I(self, linear_model_mult_constant=None, resource=1):
        linear_model_mult_constant = (
            self.linear_model_mult_constant if linear_model_mult_constant is None else linear_model_mult_constant
        )
    ...
    def monod(self, monod_mult_constant=None, half_conc=None, resource=1):
        monod_mult_constant = (
            self.monod_mult_constant if monod_mult_constant is None else monod_mult_constant
        )
        half_conc = self.half_conc if half_conc is None else half_conc
        monod_res = monod_mult_constant * resource / (half_conc + resource)
        logging.debug(
            "The half concentration is {} {} and the multiplicative monod_mult_constant is {}".format(
                half_conc, self.units, monod_mult_constant
            )
        )
        return monod_res
